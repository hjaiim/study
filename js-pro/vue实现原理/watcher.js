 ...
 * 这里还有一个细节点需要处理，我们只要在订阅者Watcher初始化的时候才需要添加订阅者，
 * 所以需要做一个判断操作，因此可以在订阅器上做一下手脚：在Dep.target上缓存下订阅者，
 * 添加成功后再将其去掉就可以了。订阅者Watcher的实现如下：
 */
...
Watcher.prototype = {
    ...
    run: function () {
        var value = this.vm.data[this.exp]; // 获取最新值
        var oldVal = this.value;    // 旧值
        if (value !== oldVal) { // 值不等,调用回调,修改dom
            this.value = value;
